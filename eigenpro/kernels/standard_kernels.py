...
def euclidean(
    samples: torch.Tensor, centers: torch.Tensor, squared: bool = True
) -> torch.Tensor:
...
def gaussian(
    samples: torch.Tensor, centers: torch.Tensor, bandwidth: float
) -> torch.Tensor:
    """Gaussian kernel.
...
def dispersal(
    samples: torch.Tensor, centers: torch.Tensor, bandwidth: float, gamma: float
) -> torch.Tensor:
    """Dispersal kernel.
...
def ntk_relu(
    X: torch.Tensor, Z: torch.Tensor, depth: int = 1, bias: float = 0
) -> torch.Tensor:
    """NTK Relu Kernel.
