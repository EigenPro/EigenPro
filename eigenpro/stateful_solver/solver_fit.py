...
from eigenpro.utils.tensor import SingleDeviceTensor
...
def fit(model, X, Y, x=None, y=None, dtype=torch.float32, kernel=None,
    ...
    # preconditioner
    nys_model_ids = model.device_offsets[device_manager.base_device_idx] + SingleDeviceTensor( # make sure all model nystrom centers are on base device 
        torch.randperm(
            device_manager.chunk_sizes(model.size)[device_manager.base_device_idx], 
            generator=torch.Generator(device=device_manager.base_device))[:model_preconditioner_size]
        )

    nys_model = model.centers[nys_model_ids.to(device_manager.base_device)].to(device_manager.base_device)

    nys_data_ids = torch.randperm(len(X), generator=torch.Generator(device=device_manager.base_device))[:data_preconditioner_size]
    nys_data = X[nys_data_ids, :]
    nys_data_base = nys_data.to(device_manager.base_device)
    nys_data = device_manager.scatter(nys_data)
    ...
    model_preconditioner.normalized_eigenvectors_broadcast = model.device_manager.broadcast(model_preconditioner.normalized_eigenvectors)
