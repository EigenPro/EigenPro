...
def asm_eigenpro_fn(samples, map_fn, top_q, bs_gpu, alpha, min_q=5, seed=1):
    ...
    print(f'eigenpro2 diag_t:{diag_t}')
...
class FKR_EigenPro(nn.Module):
    ...
    def __init__(self, kernel_fn, centers, y_dim, verbose=False, device="cuda"):
        super(FKR_EigenPro, self).__init__()
        self.kernel_fn = kernel_fn
        # ipdb.set_trace()
        self.n_centers, self.x_dim = centers.shape[0],32*32*3
        self.device = device
    ...
    def _compute_opt_params(bs, bs_gpu, beta, top_eigval):
        ...
        if bs < beta / top_eigval + 1:
            eta = bs / beta ##add devide by 2
        else:
            eta = 10*0.99 * 1 * bs / (beta + (bs - 1) * top_eigval) ##add devide by 2

        return bs, utils.float_x(eta)

    def eigenpro_iterate(self, samples, x_batch, y_batch, eigenpro_fn,
                         eta, sample_ids, batch_ids,reuse_batch=False,n_reuse=5):

        if reuse_batch:
            kzxbatch = self.kernel_matrix(x_batch)
            ksxbatch = self.kernel_fn(x_batch, samples)
            for _ in range(n_reuse):
                # update random coordiate block (for mini-batch)
                pred = kzxbatch.mm(self.weight)
                grad = pred - y_batch
                self.weight.index_add_(0, batch_ids, -eta * grad)

                # update fixed coordinate block (for EigenPro)
                correction = eigenpro_fn(grad, ksxbatch)
    ...
    def fit(self, x_train, y_train, x_val, y_val, epochs, mem_gb,
            ...
            n_train_eval=5000, run_epoch_eval=True, scale=1, seed=1,reuse_batch=False,n_reuse=5):
        ...
        prepare_time = time.time() - elapsed_start
        for epoch in epochs:
            start = time.time()
            for _ in range(epoch - initial_epoch):
                ...
                for batch_ids in np.array_split(epoch_ids, n_samples / bs):
                    ...
                    self.eigenpro_iterate(samples, x_batch, y_batch, eigenpro_f,
                                          eta, sample_ids, batch_ids,reuse_batch=reuse_batch,n_reuse=n_reuse)
            ...
            if run_epoch_eval:
                ...
                print("train accu: %.2f%%\tval accu: %.2f%% (%d epochs, %.3f seconds, %.3f seconds)\t"
                      "train l2: %.2edB\tval l2: %.2edB" %
                      ((tr_score['multiclass-acc']) * 100,
                      (tv_score['multiclass-acc']) * 100,
                      epoch, train_sec+prepare_time,elapsed, 10*np.log10(tr_score['mse']), 10*np.log10(tv_score['mse'])))
