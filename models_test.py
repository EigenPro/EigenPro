...
class TestKernelMachine(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        torch.testing.assert_close(self.km_sample(x), expected_output)
    ...
    def test_forward_sharded(self):
        x = torch.tensor([[1.0, 1.0]])
        out = self.ShardedKernelMachine(x)
        expected_output = []
        for d in self.device.devices:
    ...
    def test_shallow_copy(self):
        ...
        torch.testing.assert_close(
            copy_machine(torch.tensor([[1.0, 1.0]])),
            self.machine(torch.tensor([[1.0, 1.0]])))
